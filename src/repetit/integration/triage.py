...
3. Текст кастомный ПО СМЫСЛУ, а не пересказ заявки: НЕ повторяй то, что
   клиент и так знает про себя (имя, возраст, класс, уровень, цель —
   «Мария в 7 классе», «задача понятна», «Алексу 6 лет» и т.п. — запрещено).
   Сразу скажи, чем поможешь и как будешь строить занятия; деталь из заявки
   упоминай только если она нестандартная и по ней есть что сказать по делу.
4. Имя клиента — только в приветствии и только если оно есть в заявке
   («Здравствуйте, Мария!»), дальше пиши нейтрально. 3–6 предложений.
...
- запретные штампы: «важно отметить», «стоит подчеркнуть», «кроме того»,
  ...
  «С уважением», «Надеюсь, это поможет», «задача понятна», «задача ясна»,
  «всё понятно», «всё ясно», «step by step», «шаг за шагом»,
  «чтобы X не пугал», «опора на школьную программу», «закрываем пробелы»;
